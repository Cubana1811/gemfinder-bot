...
def fetch_oi_history(symbol):
    data = safe_get("%s/v5/market/open-interest?category=linear&symbol=%s&intervalTime=1h&limit=3" % (
        BYBIT_BASE, symbol))
    if data and data.get("retCode") == 0:
...
def fetch_funding(symbol):
    data = safe_get("%s/v5/market/funding/history?category=linear&symbol=%s&limit=1" % (BYBIT_BASE, symbol))
    if data and data.get("retCode") == 0:
...
def fetch_klines_15m(symbol, limit=10):
    data = safe_get("%s/v5/market/kline?category=linear&symbol=%s&interval=15&limit=%d" % (
        BYBIT_BASE, symbol, limit))
    if data and data.get("retCode") == 0:
        return list(reversed(data["result"]["list"]))
    return []
...
def fetch_price(symbol):
    data = safe_get("%s/v5/market/tickers?category=linear&symbol=%s" % (BYBIT_BASE, symbol))
    if data and data.get("retCode") == 0:
...
async def scan_coin(symbol, cd, bot):
    ...
    if not is_on_cooldown(cd, cd_key_trade):
        ...
        for t in trades:
            ...
            if usd_val >= WHALE_TRADE_USD:
                side = t.get("side", "Buy") == "Sell"  # True means sell
    ...
    if not is_on_cooldown(cd, cd_key_oi):
        ...
        time.sleep(0.05)
        if len(oi_data) >= 2:
            old_oi = float(oi_data[0].get("openInterest", 1))
            new_oi = float(oi_data[-1].get("openInterest", 1))
