"""
Memecoin Scanner — automatically scans new tokens on ETH, BSC, Base, Solana, and HyperEVM.

Runs every 5 minutes. For every new token found it runs 26 checks across:
  - GoPlusLabs API  (security: honeypot, taxes, functions, ownership) — EVM chains
  - Pump.fun API    (Solana token security and market data)
...
Commands:
  /scan ADDRESS [CHAIN]  — manually scan any token address
                           chain: eth, bsc, base, solana, hyperevm (default: auto-detect)
...
PUMPFUN_TOKEN     = "https://api.pump.fun/token/%s"

CHAIN_MAP = {
    "ethereum": {"goplus_id": "1",    "label": "ETH"},
    "bsc":      {"goplus_id": "56",   "label": "BSC"},
    "base":     {"goplus_id": "8453", "label": "BASE"},
    "solana":   {"goplus_id": "solana", "label": "SOLANA"},
    "hyperevm": {"goplus_id": "13234", "label": "HYPEREVM"},
}
...
def fetch_pumpfun(address):
    data = safe_get(PUMPFUN_TOKEN % address)
    if not data:
        return None
    return data
...
def analyse_token(address, chain_id, chain_label, is_solana=False):
    """
    Returns (passed, grade, score, details, hard_fail_reason) or None on API error.
    """
    if is_solana:
        sec = None
        pumpfun = fetch_pumpfun(address)
        pair = pumpfun if pumpfun else None
    else:
...
async def scan_loop(bot):
    ...
    while True:
        ...
        for token in tokens:
            ...
            name   = pair.get("baseToken", {}).get("name",   "Unknown") if "baseToken" in pair else "Unknown"
            symbol = pair.get("baseToken", {}).get("symbol", "???")     if "baseToken" in pair else "???"

            log.info("Analysing: %s (%s) on %s" % (name, symbol, chain_label))

            result = analyse_token(address, chain_id, chain_label, is_solana=is_solana)
            time.sleep(0.5)
...
async def handle_scan(bot, chat_id, args):
    ...
    await bot.send_message(chat_id=chat_id,
                           text="Scanning %s on %s..." % (address, chain_label))
    ...
    name   = pair.get("baseToken", {}).get("name",   "Unknown") if pair and "baseToken" in pair else "Unknown"
    symbol = pair.get("baseToken", {}).get("symbol", "???")     if pair and "baseToken" in pair else "???"
...
async def main():
    ...
    await bot.send_message(
        ...
        text=(
            ...
            "Auto-scanning ETH, BSC, Base, Solana, and HyperEVM\n"
            ...
            "Commands:\n"
            "  /scan ADDRESS [eth/bsc/base/solana/hyperevm]\n"
